...
def load_data(_ticker):
    ...
    for attempt in range(tries):
        try:
            ...
            if historicData is None or historicData.empty:
    ...
    st.error("Failed to fetch data from Yahoo Finance after multiple attempts.")
    return pd.DataFrame()
...
def business_news_feed():
    ...
    st.header('NEWS FEED')
    try:
        r = requests.get('https://newsapi.org/v2/top-headlines?country=' + countries[select_country] + '&category=business&apikey=' + api_key, timeout=10)
        ...
        st.error("Failed to fetch news. Please check your internet connection or API availability.")
...
def stockPricesToday():
    ...
    priceChangeToday = selection.financial_data[selected_stock]['currentPrice'] - selection.summary_detail[selected_stock]['open']
    col1.metric(label="Current Price, Change w.r.t Opening Price", value='%.2f' % selection.financial_data[selected_stock]['currentPrice'], delta='%.2f' % priceChangeToday)

    priceChangeYesterday = data['close'][len(data) - 1] - data['close'][len(data) - 2] if len(data) >= 2 else 0
    col2.metric(label="Previous Closing, Previous Day Change", value='%.2f' % data['close'][len(data) - 1], delta='%.2f' % priceChangeYesterday)
...
try:
    ...
    selection = Ticker(selected_stock, timeout=10)
    data = load_data(selection)

    if data.empty:
        st.warning("No historical data found. Try a different stock symbol.")
    else:
